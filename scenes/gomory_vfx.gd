...
func play_short():
	...
	EQ3.text = str(snapped(data["GMIaLatticex"], 0.001)) + "x" + eq3_y_sign + str(abs(snapped(data["GMIaLatticey"], 0.001))) + "y" + " = " + str(snapped(data["GMIb"], 0.001))
	# make the lines
	...
	# line 3 goes from data["point1"] to data["point2"]
	LINE3.global_position = data["point1"] * data["scaling"] + data["offset"]
	LINE3.rotation = (data["point2"] - data["point1"]).angle()
